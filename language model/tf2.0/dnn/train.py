...
def evaluate_full(test_data, model, model_path, batch_size, item_cate_map, save=True, coef=None):
    ...
    if save:
        return {'recall': recall, 'ndcg': ndcg, 'hitrate': hitrate, 'acc': acc}
    return {'recall': recall, 'ndcg': ndcg, 'hitrate': hitrate, 'diversity': diversity, 'acc': acc}
...
def train(train_file,
    ...
    best_model_path = "%s/best_model/model.ckpt"%args.model_path
    ...
    with tf.device('/gpu:0'):
        ...
        for epoch in range(1000):
            ...
            for src, tgt in train_data:
                ...
                if iter % test_iter == 0:
                    ...
                    log_str = 'epoch: %d, iter: %d, train loss: %.4f' % (epoch, iter, loss_sum / test_iter)
                    if metrics != {}:
                    ...
                    if 'recall' in metrics:
                        ...
                        if trials == 8 and recall_flag < 2 and recall <= 0.10 :
                            print("reset adam, recall=%f"%best_metric)
                            recall_flag += 1
                            trials = 0
                            if recall_flag == 1:
                                ...
                                print("Adam learning_rate=0.0005")
            ...
            if trials > patience:
                break
            if best_metric > 0.101:
                break
        print("begin test and load best model")
        save_model = dnn_model(item_count, args.embedding_dim, args.hidden_size, batch_size, args.num_interest, maxlen)
        save_model.load_weights(best_model_path)
        print("end load best model and begin eval")
        eval_file = "%s/eval_result"%args.model_path
        eval_f = open(eval_file, 'w')
        eval_f.write("%s\n"%eval_file)
        metrics = evaluate_full(valid_data, save_model, best_model_path, batch_size, item_cate_map, save=False)
        info = ', '.join(['valid ' + key + ': %.6f ' % value for key, value in metrics.items()])
        ...
        info = ', '.join(['test ' + key + ': %.6f ' % value for key, value in metrics.items()])
...
if __name__ == '__main__':
    ...
    maxlen = 20
    test_iter = 500
    train_file = "%s/train.txt"%args.data_path
    valid_file = "%s/valid.txt"%args.data_path
    test_file = "%s/test.txt"%args.data_path
    ...
    if args.p == 'train':
        ...
        train(train_file=train_file, valid_file=valid_file, test_file=test_file, cate_file=cate_file,
              item_count=item_count, batch_size=batch_size, maxlen=maxlen, test_iter=test_iter, 
              lr=args.learning_rate, max_iter=args.max_iter, patience=args.patience)
    elif args.p == 'test':
        test(test_file=test_file, cate_file=cate_file, item_count=item_count, batch_size=batch_size,
             maxlen=maxlen, lr=args.learning_rate)
    elif args.p == 'output':
        output(item_count=item_count, batch_size=batch_size, maxlen=maxlen, 
